...
**練習3:**
最小二乗法による回帰分析の手順として正しいものはどれか。

1.データ入力→設計行列作成→パラメータ推定→予測→誤差評価<br>
2.データ入力→パラメータ推定→設計行列作成→予測→誤差評価<br>
3.データ入力→予測→設計行列作成→パラメータ推定→誤差評価<br>
4.データ入力→予測→パラメータ推定→設計行列作成→誤差評価<br>
5.データ入力→設計行列作成→予測→パラメータ推定→誤差評価<br>
...
**練習4:**
最尤推定と最小二乗法の関係について正しいものはどれか。

1. 最小二乗法は一般に最尤推定より予測精度が高い<br>
2. 誤差が一様分布に従うとき両者は一致する<br>
3. 誤差が正規分布に従うとき最尤推定は最小二乗法と一致する<br>
4. 誤差分布に関係なく両者は常に一致する<br>
5. 最小二乗法は確率モデルを仮定しない最尤推定である<br>
...
最後の4は４次の多項式（$y=a+bx+cx^2+dx^3+ex^4$）を用いるということを意味する
