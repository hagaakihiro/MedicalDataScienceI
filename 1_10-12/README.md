# 最小二乗法と重回帰分析
これまで学んできた最小二乗法（広い意味では最尤推定、事後分布最大化も含む）は説明変数と目的変数が１つずつでした。
...
**ただし、次元が増えすぎると別の問題が生じることもわかっている（次元の呪い, 多重共線性, etc）*
...
これまで学んできた最小二乗法と最尤推定、さらに正則化やその枠組みを統一的に捉える事後分布最大化法は、多変量の入力（説明変数）・出力（目的変数）を包含した理論である。つまり、これまで一変数入力ー一変数出力というものだけではなく多変量入力ー多変量出力を可能にする手法である。ここでは、多変量に拡張した枠組みで最小二乗法をもう一度見直し、演習では多変量入力である数値を予測する手法をプログラミングで実行する。データには全前立腺切除を受けた97人の男性について、前立腺特異抗原（PSA）の値と臨床的な8つの変数との関係性を調査したStamey (1987)のデータを使用し、８つの説明変数からPSAを予測するモデルを作成する。
...
**練習4:**
重回帰分析の説明として正しいものはどれか。

1.単一の説明変数から複数の目的変数を予測する手法である<br>
2.誤差分布を仮定しない手法である<br>
3.目的変数はカテゴリ変数である<br>
4.識別（分類）問題専用の手法である<br>
5.上記のどれでもない
...
（ヒント）dataの中に含まれる項目（説明変数）に対して相互相関係数を全て計算するには以下を使う。
```py
# 相関係数行列の計算
...
説明変数間の相関は何を意味するだろうか？
極端な場合を考えるとその意味が掴めてくる。
例えば、相関が1である説明変数のペアがあったとしよう。そうすると、両方を説明変数として使うのは意味がないことに気づく（$x_1$と$x_2$が比例するので、どちらかの係数に組み込んでしまうことができる）。したがって、どちらかを除いても予測モデルの精度に影響がない。説明変数を減らすことができるのでむしろ予測値の推定が安定となる（練習2でやったように、扱う行列の行列式が0とならない。自分で確かめてみよう）。相関が高い変数ペアがある場合には、そのどちらかを除くと冗長性が減り、推定も安定する。
このように説明変数間の相関を**多重共線性**という。
...
(2) LASSO回帰で、定数項を除いて係数が0にならない数が5つの時の最小のlambを探せ。<br>
