...
それぞれのデータ
（lcavol,lweight, age, lbph, svi, lcp, gleason, pgg45）の値の大きさやばらつきが異なるので、それぞれのデータをその標準偏差で割ることで標準化を実施する。
```py
    input_name = ['lcavol', 'lweight', 'age', 'lbph', 'svi', 'lcp', 'gleason', 'pgg45']
    
    for name0 in input_name:
        df[name0] = df[name0]/df[name0].std()
```
...
データの標準化についても、以下のように平均値を引いたもので行ってみよう。
```py
#---- main ----
if __name__ == '__main__':
...
```
($(x-\mu)/\sigma$という形で標準化する。前回は平均値を引かなかった。)
...
(2) Ridge回帰で出力されるDOFは有効自由度と呼ばれるものである。意味するところは、何個の変数がlpsaの予測に有効であったかを示す指標である。DOFが5の時になるlambの値を探せ。<br>
(3) LASSO回帰で、係数が0にならない数が3つの時の最小のlambを探せ。