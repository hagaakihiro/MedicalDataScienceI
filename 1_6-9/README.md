...
**練習2:**
正則化最小二乗法の目的関数として正しいものはどれか。

1.$E=\sum_{n=1}^{N}(y_n-\hat{y}_n)^2$<br>
2.$E=\sum_{n=1}^{N}(y_n-\hat{y}_n)^2+\lambda\sum_{i=1}^{M}|w_i|$<br>
3.$E=\sum_{n=1}^{N}|y_n-\hat{y}_n|$<br>
4.$E=\prod_{n=1}^{N}(y_n-\hat{y}_n)^2+\lambda\sum_{i=1}^{M}|w_i|$<br>
5.$E=\sum_{n=1}^{N}(y_n-\hat{y}_n)+\lambda\sum_{i=1}^{M}|w_i|$<br>
...
### 練習7
* (1)Data_2026.csvを読み込んで$x$と$t$の散布図を作成せよ（これは12MeV電子線のPercent Depth Dose（PDD）である。depthの単位は[cm]である。）。<br>
読み込みにおいて
df = pd.read_csv("Data_2026.csv", encoding="SHIFT-JIS")
でエラーが出る場合は
df = pd.read_csv("Data_2026.csv", encoding="utf-8")
とすること。
...
これは定理（証明されている）であり暗記して良い（具体的な例を持ってこれらが成り立つことを確認しておくとよい）。
...
**練習8:**
パラメータ $\theta$ に対する尤度関数 $L(\theta)=p(D|\theta)$ と事前分布 $p(\theta)$ を用いたとき，事後分布として正しいものはどれか。$D$はデータとする。

1.$p(\theta|D)=p(D|\theta)p(\theta)$<br>
2.$p(\theta|D)=\dfrac{p(D|\theta)p(\theta)}{p(D)}$<br>
3.$p(\theta|D)=\dfrac{p(\theta)}{p(D|\theta)}$<br>
4.$p(\theta|D)=p(D)p(\theta)$<br>
5.$p(\theta|D)=\dfrac{p(D)}{p(\theta)}$<br>


**練習9:**
パラメータ値$\theta$に対する事後分布最大化（MAP推定）として正しいものはどれか。$D$はデータとする。

1.$\hat{\theta}=\arg\max_{\theta} p(D|\theta)$<br>
2.$\hat{\theta}=\arg\max_{\theta} p(\theta|D)$<br>
3.$\hat{\theta}=\arg\max_{\theta} p(\theta)$<br>
4.$\hat{\theta}=\arg\min_{\theta} p(D|\theta)p(\theta)$<br>
5.$\hat{\theta}=\arg\min_{\theta} p(\theta|D)$<br>
