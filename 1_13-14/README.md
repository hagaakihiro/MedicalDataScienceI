...
その対局として、ホールドアウト法がある。一回だけ訓練と検証データを分割する方法である。
...
**練習1**
機械学習における「訓練・検証・テスト」のデータ分割においてテスト用データ（test data）の役割として正しいものはどれか。

1.最終的な性能評価のみに使用する<br>
2.学習データと同じ重み更新に使用する<br>
3.教師ラベルを生成するために使用する<br>
4.データの前処理を行うためだけに使用する<br>
5.ハイパーパラメータの調整やモデル選択に使用する<br>
...
$$
p(\vec{x}) = {\cal N}(\vec{x}|\vec{\mu}, \Lambda^{-1}) \hspace{2mm}\cdots(3)<br>
...
となる。ここで$\Phi$は、これも「３線形回帰モデルと正則化」で与えている計画（設計）行列である。
（面倒な計算が入るのでここでは結果だけを示すが、皆さんであれば応用数学などで既に習っている代数を使うことでこのように書けることを証明できます。）
